...
async def _create(admin_client: AsyncClient, **payload) -> dict:
...
class TestSlugGeneration:
    async def test_slug_is_derived_from_the_name_when_omitted(self, admin_client: AsyncClient):
        hotel = await _create(admin_client, name="Dağ & Deniz — Butik")
        assert hotel["slug"] == "dag-deniz-butik"

    async def test_an_explicit_slug_is_kept(self, admin_client: AsyncClient):
        hotel = await _create(admin_client, name="Cave Hotel", slug="magara-oteli")
        assert hotel["slug"] == "magara-oteli"

    async def test_a_name_with_no_slugable_characters_is_rejected(self, admin_client: AsyncClient):
        response = await admin_client.post("/hotels", json={"name": "!!! ###", "city": "Test"})
        assert response.status_code == 422
        assert "slug" in response.json()["detail"].lower()

    async def test_a_duplicate_slug_conflicts(self, admin_client: AsyncClient):
...
class TestLookup:
    async def test_a_hotel_is_reachable_by_slug_and_by_uuid(self, admin_client: AsyncClient):
    ...
    async def test_an_unknown_slug_is_a_404(self, client: AsyncClient):
        response = await client.get("/hotels/yok-boyle-bir-otel")
        assert response.status_code == 404

    async def test_an_unknown_uuid_is_a_404(self, client: AsyncClient):
        response = await client.get(f"/hotels/{uuid.uuid4()}")
        assert response.status_code == 404

    async def test_a_hotel_with_no_tours_returns_an_empty_list(self, admin_client: AsyncClient):
        created = await _create(admin_client, name="Cave Hotel & Spa")
...
class TestUpdateAndDelete:
    ...
    async def test_resending_a_hotels_own_slug_is_not_a_conflict(self, admin_client: AsyncClient):
    ...
    async def test_taking_another_hotels_slug_is_a_conflict(self, admin_client: AsyncClient):
...
class TestListing:
    async def test_the_plain_list_is_an_array(self, admin_client: AsyncClient):
        await _create(admin_client, name="Cave Hotel & Spa")
        response = await admin_client.get("/hotels")
        assert response.status_code == 200
        assert isinstance(response.json(), list)

    async def test_paging_returns_the_refine_shape(self, admin_client: AsyncClient):
