    ...
    <div className="min-h-screen bg-white-token flex flex-col font-sans text-main-token">
      ...
      <section className="bg-white-token border-b border-token py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto space-y-4">
          <div className="flex items-center gap-3">
            <span className="badge-cyan-token font-bold text-xs font-mono">
              {service.codeNumber}
            </span>
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-12">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-sm">
                {service.specifications.map((spec, idx) => (
                  <div
                    key={idx}
                    className="flex justify-between border-b border-token pb-1.5 text-subtle-token"
                  >
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {service.faqs.map((faq, idx) => (
                <div
                  key={idx}
                  className="card-token p-6 bg-white-token border-2 border-cyan-token space-y-2"
                >
        ...
        <div className="pt-8 border-t border-token space-y-6">
          <h2 className="text-xl font-bold text-main-token">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {servicesData
              ...
              .map((other) => (
                ...
                >
                  <div className="text-xs font-bold text-subtle-token font-mono mb-1">
                    {other.codeNumber}
                  </div>
                  <h3 className="font-bold text-sm text-main-token">{other.title}</h3>
                  <span className="text-xs font-semibold text-brand-token mt-2">
                    Detayları İncele →
                  </span>
                </Link>
              ))}
