...
- `conftest.py` overrides **only** `get_current_user`. `get_current_superuser` depends on it, so the real privilege check still runs and non-superusers genuinely get 403 from admin routes. Do not override both. The token/password flow itself is still uncovered.
