...
- **Audit trail.** `audit_logs` records who moved money or seats: booking created/confirmed/cancelled/expired and payment opened/paid/refunded, with the amount, the actor and a JSONB `detail`. Written through `audit_service.record`, read at `GET /admin/audit-logs` (superuser only, filters: `action`, `booking_id`, `payment_id`, `actor_id`) and shown in the admin panel under "Denetim Günlüğü". Rules that are load-bearing:
  - **`record` adds to the caller's session and does not commit.** The entry rides on the same transaction as the change it describes, so a rolled-back operation leaves no line claiming it happened and a committed one cannot be missing its line. Recording *after* a commit writes nothing at all.
  - **No foreign keys, anywhere in the table.** Payments are deleted with their booking (`ON DELETE CASCADE`) and accounts can be removed; a trail wired up the same way would vanish exactly when somebody wants to read it. The actor's email is snapshotted at write time for the same reason.
  - **Order by `seq`, never by time.** `recorded_at` uses `clock_timestamp()` (not `CURRENT_TIMESTAMP`, which is the transaction's start time), but several entries are still written inside one transaction — paying records the charge and the confirmation together — so the identity column is what gives a stable order.
  - **Read-only by design.** No update path, no delete endpoint, and no admin-panel affordance for either. A log an administrator can tidy up is not evidence of anything.
  - `test_audit_api.py` cannot prove an entry was *committed*: its session both writes and reads, and SQLAlchemy autoflushes before a query, so a pending entry shows up there. `test_concurrency.py::TestTheAuditTrailIsActuallyCommitted` reads back on a separate connection — that is the test that fails when recording moves after the commit. Its `world` fixture also deletes audit rows in teardown, since nothing cascades them away.
  - Services that move money take an `actor: User` (or `User | None` for the sweeper, which has nobody behind it). Do not default it — an unattributed entry is barely worth writing.
