...
- `GET /tours` (unpaginated only) falls back to `DEFAULT_TOURS`, a hard-coded demo catalogue, when no active tour exists — so an empty or fully unpublished database shows customers trips that are not real. `tests/test_tours_api.py` pins this so it cannot change silently. It is pinned, not endorsed; decide deliberately before launch.
